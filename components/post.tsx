...
import { AlertCircle, AlertTriangle, BookmarkIcon, Heart, MessageCircle, MoreHorizontal, Send } from "lucide-react"
    ...
    <Card>
      {post.status === "flagged" && (
        <Alert className="border-yellow-500 bg-yellow-50 dark:bg-yellow-950 rounded-b-none">
          ...
          <AlertTitle className="text-yellow-700 dark:text-yellow-300 text-sm">Flagged Content</AlertTitle>
          <AlertDescription className="text-yellow-600 dark:text-yellow-400 text-xs">
            {post.reason || "This content has been flagged for potential cyberbullying indicators."}
          </AlertDescription>
        </Alert>
      )}
      ...
      {post.status === "blocked" && (
        <Alert className="border-red-500 bg-red-50 dark:bg-red-950 rounded-b-none">
          ...
          <AlertTitle className="text-red-700 dark:text-red-300 text-sm">Blocked Content</AlertTitle>
          <AlertDescription className="text-red-600 dark:text-red-400 text-xs">
            {post.reason || "This content contains cyberbullying indicators and would be blocked."}
          </AlertDescription>
        </Alert>
      )}
      ...
      <CardFooter className="flex flex-col p-4 pt-2 space-y-4">
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs">{liked ? post.like_count + 1 : post.like_count}</span>
          </div>
          <div className="flex items-center space-x-2">
            <Button variant="ghost" size="icon" className="h-8 w-8">
        ...
        <form onSubmit={handleComment} className="flex w-full items-center space-x-2">
          <Input
            type="text"
          ...
          />
          <Button type="submit" size="icon" className="h-8 w-8" disabled={!comment.trim()}>
